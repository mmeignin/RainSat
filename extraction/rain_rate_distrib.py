...
with tqdm(total=len(all_images), desc="Processing Images", unit="file") as pbar:
    for i, path in enumerate(all_images):
        ...
        cumul += np.where(imagette_mask > 0.1, np.ones_like(imagette_mask), 0)
...
plt.title("Rain Rate Over France")
plt.colorbar(p, ax=ax, label="Rain Rate distribution")

# Save and display the plot
plt.savefig(f"rain_rate_distribution{proj_match}.png", dpi=300)
plt.close()
